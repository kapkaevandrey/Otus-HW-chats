...
class KafkaActionConsumer(KafkaConsumer):
    def __init__(
        self,
        consumer_class: type[AIOKafkaConsumer],
        consumer_args: tuple,
        consumer_kwargs: dict,
        schema: type[CUDMessageValue],
        process_retries: int = 1,
        logger: logging.Logger | None = None,
    ):
        """Async kafka consumer, which implements basic SAT kafka consumer features
        (such as process retries, asynccontextmanager, automatic start/stop, etc.)
        and also has methods to analize message action and multiplex message
        processing.
        ...
        Args:
            ...
            process_retries (int, optional): number attempts to process incoming message.
                 After provided number of failures `RetriesExceeded` exception will be raised. Defaults to 1.
            schema (type[BaseCUDMessageValue], optional): schema to parse incoming message.
                 Must implement `get_action` method to work properly. Defaults to CUDMessageValue.
            logger (logging.Logger, optional): Logger instance. Defaults to default logger.
        """
        super().__init__(consumer_class, consumer_args, consumer_kwargs, process_retries, logger)
        self._schema = schema

    async def process_message(self, message: ConsumerRecord, context: Any = None) -> None:
        """Message handle function. Basic realization allows to define
        `f"_handle_action_{action_lower_name}"` functions to handle each action
        type.

        Example:
            Handle function example. Example is valid to all variants of `create` (`CREATE`, `Create`, `create`, etc.).

        Args:
            message (kafka.protocol.message.Message): message read from kafka
            context (Any, optional): Context provided by `self.context_manager` function. Defaults to None.
        """
        self.logger.debug(f"Message {message} | {message.key}")
        message_value = self._schema.model_validate(message.value)
        action = message_value.get_action()
        if hasattr(self, f"_handle_action_{action.lower()}"):
            await getattr(self, f"_handle_action_{action.lower()}")(message.key, message_value, context)
